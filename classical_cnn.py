...
def classical_cnn(dim, output_neurons, output_activation):
    ...
    def block(tensor, conv_reps, n_filters):
        x = Conv2D(filters = n_filters, kernel_regularizer=l2(lmbd), bias_regularizer=l2(lmbd), kernel_size = (3,3), padding = 'same')(tensor)
        x = LeakyReLU()(x)
        
        for i in range(conv_reps-1):
            x = Conv2D(filters = n_filters, kernel_regularizer=l2(lmbd), bias_regularizer=l2(lmbd), kernel_size = (3,3), padding = 'same')(x)
            x = LeakyReLU()(x)
    ...
    for i in range(dense_reps):
        x = Dense(128, kernel_regularizer=l2(lmbd), bias_regularizer=l2(lmbd))(x)
        x = LeakyReLU()(x)
        x = Dropout(0.5)(x)
